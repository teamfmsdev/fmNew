<template>
  <b-modal ref="scheduleModal" class="modalTable" centered id="scheduleModal">
    ...
    <div class="loadingBar fixed-top"  v-show="getIsLoading">
      <b-spinner
        variant="warning"
        type="grow"
        label="Spinning"
    ...
    <b-table
   :tbody-tr-class="currentDay" hover small class="mt-3" bordered :items="listOfDays" :fields="field">
...
export default {
  ...
  methods: {
    ...
    currentDay (item) {
      ...
      let rowDate = dayjs(new Date(item.fullDate))

      return today.isSame(rowDate, 'date') ? 'currentDay' : ''
    }
  },
  created () {
    this.generatedDays()
